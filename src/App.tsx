...
export function App() {
  ...
  useEffect(() => {
    ...
    const wheel = (event: WheelEvent) => {
      ...
      send({ type: 'GROWTH_SCROLL', delta: Math.min(.16, Math.abs(event.deltaY) / 6500) })
    }
    ...
    const touchMove = (event: TouchEvent) => {
      ...
      if (Math.abs(distance) > 3) {
        send({ type: 'GROWTH_SCROLL', delta: Math.min(.12, Math.abs(distance) / 1800) })
  ...
  return (
    <main className={`experience scene-${scene}`}>
      ...
      <FlowerField active={scene === 'bloom' || scene === 'question' || scene === 'accepted' || scene === 'first-day'} progress={scene === 'bloom' ? story.growthProgress : 1} />
