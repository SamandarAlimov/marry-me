...
export interface WindField { x: MotionValue<number>; y: MotionValue<number>; strength: MotionValue<number>; targetX: MotionValue<number>; targetY: MotionValue<number> }

export function useWindField(): WindField {
  const targetX = useMotionValue(0)
  const targetY = useMotionValue(0)
  const x = useSpring(targetX, { stiffness: 26, damping: 18, mass: .7 })
  const y = useSpring(targetY, { stiffness: 26, damping: 18, mass: .7 })
  const strength = useTransform([x, y], ([currentX, currentY]: number[]) => Math.min(1, Math.hypot(currentX, currentY)))
