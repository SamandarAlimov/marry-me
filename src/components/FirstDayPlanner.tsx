...
const ACTIVITIES: Choice[] = [
  { id: 'conversation', label: 'Uzoq suhbat', meta: 'Bir-birimizni yana yaxshiroq tanish uchun' },
  { id: 'quran-time', label: 'Birga Qur’on o‘qish', meta: 'Kunning bir qismini xotirjamlikka ajratamiz' },
  { id: 'dua', label: 'Birga duo qilish', meta: 'Yangi yo‘l uchun Allohdan yaxshilik so‘raymiz' },
  { id: 'surprise', label: 'O‘sha paytda hal qilamiz', meta: 'Rejada ozgina joy kutilmagan narsalar uchun qolsin' },
]
const TIMES: Choice[] = [
...
export function FirstDayPlanner() {
  ...
  const steps: Array<{ key: string; title: string; choices: Choice[] }> = [
    ...
    { key: 'meaning', title: 'Bu kunning mazmuni nima bo‘lsin?', choices: ACTIVITIES },
  ...
  const back = () => { if (step > 0) setStep((value) => value - 1) }
  ...
  return <section className="planner" aria-label="Birinchi kun rejalashtirgichi">
    ...
    <AnimatePresence mode="wait">
      {!done && current ? <motion.div key={`${current.key}-${step}`} className="planner-step" initial={{ opacity: 0, y: 24, filter: 'blur(5px)' }} animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }} exit={{ opacity: 0, y: -18, filter: 'blur(4px)' }} transition={{ duration: .6, ease: [0.16, 1, .3, 1] }}>
        ...
        {step > 0 && <button className="planner-back" onClick={back}>← Oldingi savol</button>}
