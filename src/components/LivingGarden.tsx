...
const grass = createOrganicPoints(96, 41)

export function LivingGarden({ active, growthProgress = 1 }: LivingGardenProps) {
  const fireflies = useMemo(() => createParticles(30, 91), [])
  const wind = useWindField()
  const crownY = useTransform(wind.y, [-1, 1], [-2.8, 2.8])
  const heroLeftX = useTransform(wind.x, [-1, 1], [-5, 5])
  const heroRightX = useTransform(wind.x, [-1, 1], [-4, 4])
  const growth = Math.max(0, Math.min(1, growthProgress))
  const visibleLeaves = Math.ceil(leaves.length * Math.max(.08, growth))
  const visibleTrees = Math.ceil(trees.length * Math.max(.16, growth))
  ...
  return <motion.div className="living-garden" aria-hidden="true" initial={{ opacity: 0, y: 80 }} animate={{ opacity: active ? 1 : 0, y: active ? 0 : 80 }} transition={{ duration: 1.6, ease: [0.16, 1, 0.3, 1] }}>
    ...
    <motion.div className="hero-tree hero-tree-right" style={{ x: heroRightX, scaleY: growth }} animate={{ opacity: active ? 1 : 0 }} transition={{ duration: 1.4, delay: .18, ease: [0.16, 1, 0.3, 1] }}>
      ...
      <div className="hero-leaves hero-leaves-right">{leaves.slice(19).map((leaf, index) => <motion.i key={leaf.id} style={{ left: `${leaf.x}%`, top: `${leaf.y}%` }} initial={{ opacity: 0, scale: .1 }} animate={active && index + 19 < visibleLeaves ? { opacity: 1, scale: 1, rotate: [6, -5, 4], y: [0, 2, -1] } : { opacity: 0, scale: .1 }} transition={{ duration: 1.25, delay: leaf.delay + index * .035, repeat: active && index + 19 < visibleLeaves ? Infinity : 0, repeatType: 'mirror', repeatDelay: .15, ease: 'easeInOut' }} />)}</div>
    </motion.div>
    {trees.map((tree, index) => <motion.div key={index} className="tree" style={{ left: tree.x, scale: tree.scale }} initial={{ y: 30, opacity: 0 }} animate={{ y: active ? 0 : 30, opacity: active && index < visibleTrees ? 1 : 0, scaleY: active ? .92 + growth * .08 : .45 }} transition={{ delay: tree.delay, duration: 1.4, ease: 'easeOut' }}><div className="trunk" /><motion.div className="crown" style={{ x: useTransform(wind.x, [-1, 1], [-3 - tree.scale * 2, 3 + tree.scale * 2]), y: crownY }} animate={{ rotate: [-1, 1, -1], scale: .88 + growth * .12 }} transition={{ duration: 5 + tree.scale, repeat: Infinity, ease: 'easeInOut', delay: tree.delay }}><i /><i /><i /><i /></motion.div></motion.div>)}
